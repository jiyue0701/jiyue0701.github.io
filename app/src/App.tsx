...
function MotionPlayer({ workoutExercise, exercise, elapsedMs, paused, onReady }: { workoutExercise: WorkoutExerciseV2; exercise: Exercise; elapsedMs: number; paused: boolean; onReady?: () => void }) {
  ...
  return <div className="motion-player">
    <div className={`motion-player__video-slot${['dead-bug', 'forearm-plank', 'chair-knee-raise', 'seated-chair-march'].includes(workoutExercise.exerciseId) ? ' motion-player__video-slot--landscape' : ''}`}>{hasFormalVideo ? <video key={workoutExercise.exerciseId} ref={videoRef} data-exercise-id={workoutExercise.exerciseId} poster={workoutExercise.posterUri} autoPlay={!paused} loop muted playsInline preload="auto" onCanPlay={onReady} onLoadedMetadata={onReady}><source src={videoUri} type={videoUri?.endsWith('.mp4') ? 'video/mp4' : 'video/webm'} />{workoutExercise.videoFallbackUri && <source src={workoutExercise.videoFallbackUri} type={workoutExercise.videoFallbackUri.endsWith('.mp4') ? 'video/mp4' : 'video/webm'} />}</video> : frameUris.length ? <img key={workoutExercise.exerciseId} src={paused ? workoutExercise.posterUri : frameUris[frameIndex]} alt={motion.accessibility.altText} /> : <img key={workoutExercise.exerciseId} src={workoutExercise.posterUri} alt={`${exercise.name}动作示范`} />}</div>
  </div>
}
